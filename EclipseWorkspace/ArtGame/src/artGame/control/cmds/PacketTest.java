 ...
 *
 */
public class PacketTest {	@Test
	public void movePlayerPacket_read() throws IncompatiblePacketException {
		int recipientId = 5;
		int movingPlayerId = recipientId;
		Point current = new Point(5,4);
		long time = System.currentTimeMillis();
		MovePlayerAction mpa = new MovePlayerAction(recipientId, movingPlayerId, current, Direction.NORTH, time);
...
//					mpa.equals(readPacket));
	...
	public void movePlayerPacket_read1() {
		int recipientId = 4;
		int movingPlayerId = 10;
		Point current = new Point(12,12);
		long time = System.currentTimeMillis();
		MovePlayerAction mpa = new MovePlayerAction(recipientId, movingPlayerId, current, Direction.EAST, time);
...
//						(byte)current.getX(), 	(byte)current.getY(),(byte)1,   // packet contents
//						(byte)dest.getX(), 		(byte)dest.getY(),	 (byte)3,	// packet contents
//					(byte)Packet.TERMINAL 										// terminal
//			};
//			MovePlayerAction mpa = new MovePlayerAction(false,recipientId,movingPlayerId,current,Direction.NORTH,dest,Direction.SOUTH);
