...
public class ImageButton implements Widget {
	...
	public ImageButton(File image, float width, float height) {
		float[] vertices = {
				...
				-0.8f, 0.2f, 0f, -0.8f, -0.8f, 0f, 0.2f, -0.8f, 0f,
				// Right top triangle
				0.2f, -0.8f, 0f, 0.2f, 0.2f, 0f, -0.8f, 0.2f, 0f };
		...
		vertBufferObject.uploadBufferData(GL15.GL_ARRAY_BUFFER, vertBuffer,
				GL15.GL_STATIC_DRAW);

		uvBufferObject = new VertexBufferObject();
		uvBufferObject.bind(GL_ARRAY_BUFFER);
		uvBufferObject.uploadBufferData(GL_ARRAY_BUFFER, vertBuffer,
				GL_STATIC_DRAW);
	...
	@Override
	public void draw(float width, float height, float screenWidth,
			float screenHeight) {
