...
public class GameRenderer implements Screen {
	...
	public void render(float delta) {
		...
		if (cameraTween != null) {
			if (cameraTween.isFinished(currentTime)) {
				cameraTween = null;
				camera.setRotation(new Vector3f(CAMERA_ANGLE, 90 * (Math
						.round(currentCameraAngle / 90)), 0));
			} else {
	...
	private void resetAssets() {
		...
		playerSprite = AssetLoader.instance().loadSpritesheet(
				"res/red_player.png", 32);
		guardSprite = AssetLoader.instance().loadSpritesheet(
				"res/red_player.png", 32); // TODO Add guard sprite
	}
	...
	public void rotateLeft() {
		if (cameraTween == null) {
			cameraTween = new TweenFloat(currentCameraAngle, 0.5f, 90,
					currentTime);
		}
	}
