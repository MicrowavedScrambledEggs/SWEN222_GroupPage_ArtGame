...
import artGame.ui.renderer.AnimatedSpriteController;
...
public class GameRenderer implements Screen {
	...
	private Model sinks;
	private Model desks;
	private AnimatedSpriteController playerSprite;
	private AnimatedSpriteController guardSprite;
	private Map<Integer, Painting> paintingCatalogue;

	private Map<artGame.game.Character, Asset> characters;
	private Map<AnimatedSpriteController, artGame.game.Character> controllerToCharacter;
	...
	private Map<AnimatedSpriteController, Tween<Vector3f>> spriteTweens;
	...
	public GameRenderer() {
		resetAssets();
		controllerToCharacter = new HashMap<AnimatedSpriteController, artGame.game.Character>();
		...
		spriteTweens = new HashMap<AnimatedSpriteController, Tween<Vector3f>>();
		cameraTween = null;
	...
	public void render(float delta) {
		...
		if (SPRITE_TWEENING) {
			for (artGame.game.Character c : entityPositions.keySet()) {
				if (c.getCol() != entityPositions.get(c).getX()
					...
					entityPositions.put(c,
							new Vector3f(c.getCol(), 0, c.getRow()));
					if (spriteTweens.get((AnimatedSpriteController) characters
							.get(c)) == null) {
						spriteTweens.put(
								(AnimatedSpriteController) characters.get(c),
								new TweenVector3f(
										((AnimatedSpriteController) characters
												.get(c)).getPosition(), 0.5f,
										end, currentTime));
					}
				}
			}

			List<AnimatedSpriteController> stopTweening = new ArrayList<AnimatedSpriteController>();
			for (AnimatedSpriteController s : spriteTweens.keySet()) {
				if (spriteTweens.get(s).isFinished(currentTime)) {
					stopTweening.add(s);
					s.setPosition(spriteTweens.get(s).getEndValue());
					s.updateImage(controllerToCharacter.get(s).getDir(),
							currentCameraAngle, null);
				} else {
					s.setPosition(spriteTweens.get(s).tween(currentTime));
					s.updateImage(controllerToCharacter.get(s).getDir(),
							currentCameraAngle,
							spriteTweens.get(s).completion(currentTime));
				}
			}

			for (AnimatedSpriteController s : stopTweening) {
				spriteTweens.remove(s);
			}
		} else {
			for (artGame.game.Character c : characters.keySet()) {
				if (characters.get(c) instanceof AnimatedSpriteController) {
					((AnimatedSpriteController) characters.get(c))
							.setPosition(new Vector3f(c.getCol(), 0, c.getRow()));
					((AnimatedSpriteController) characters.get(c)).updateImage(
							c.getDir(), currentCameraAngle, null);
				}
			}
		}
		...
		camera.setPosition(((AnimatedSpriteController) characters.get(GameData
				.getPlayer())).getPosition().scale(-1));
	...
	private List<Asset> getRenderList() {
		...
		for (artGame.game.Character c : characters.keySet()) {
			if (playerTile
					...
					.getTiles()
					.contains(
							GameData.getFloor().getTile(c.getRow(), c.getCol()))) {
	...
	private Map<artGame.game.Character, Asset> loadCharacters() {
		...
		for (artGame.game.Character c : GameData.getCharacters()) {
			if (c instanceof Player) {
				...
				controllerToCharacter.put(
						(AnimatedSpriteController) chars.get(c), c);
				((AnimatedSpriteController) chars.get(c))
						.setPosition(new Vector3f(c.getCol(), 0, c.getRow()));
	...
	private Map<Integer, Painting> loadPaintings(String idFilePath) {
		...
		try {
			...
			while (scan.hasNext()) {
				temp.put(
						scan.nextInt(),
						AssetLoader.instance().loadPainting(
								"res/paintings/" + scan.next(), 64));
			}
	...
	private Map<Model, Tile> loadFullLevel() {
		...
		for (int row = 0; row < world.getHeight(); row++) {
			for (int col = 0; col < world.getWidth(); col++) {
				...
				Matrix4f pos = new Matrix4f();
				pos = pos.multiply(Matrix4f
						.translate(new Vector3f(col, 0, row)));
				...
				if (t.getWall(NORTH) != null) {
					if (t.getWall(NORTH) instanceof Door) {
						...
						if (t.getWall(NORTH).getArt() != null) {
							paintings.put(t.getWall(NORTH), paintingCatalogue
									.get(1).instantiate(pos));
						}
					}
				}
				if (t.getWall(artGame.game.Character.Direction.SOUTH) != null) {
					if (t.getWall(artGame.game.Character.Direction.SOUTH) instanceof Door) {
						...
						Matrix4f paintingPos = pos.multiply(Matrix4f.rotate(
								180, 0, 1, 0));
						if (t.getWall(SOUTH).getArt() != null) {
							paintings.put(t.getWall(SOUTH), paintingCatalogue
									.get(1).instantiate(paintingPos));
						}
					}
				}
				if (t.getWall(artGame.game.Character.Direction.EAST) != null) {
					if (t.getWall(artGame.game.Character.Direction.EAST) instanceof Door) {
						...
						if (t.getWall(EAST).getArt() != null) {
							paintings.put(t.getWall(EAST), paintingCatalogue
									.get(1).instantiate(paintingPos));
						}
					}
				}
				if (t.getWall(artGame.game.Character.Direction.WEST) != null) {
					if (t.getWall(artGame.game.Character.Direction.WEST) instanceof Door) {
						...
						Matrix4f paintingPos = pos.multiply(Matrix4f.rotate(90,
								0, 1, 0));
						if (t.getWall(WEST).getArt() != null) {
							paintings.put(t.getWall(WEST), paintingCatalogue
									.get(1).instantiate(paintingPos));
						}
					}
				}
				...
				if (t instanceof EmptyTile) {
					...
					int id = ((Chest) t).id;
					if (0 <= id && id < 20) { // loo
						forloop: for (artGame.game.Character.Direction d : artGame.game.Character.Direction
								.values()) {
							switch (d) {
							case NORTH:
								if (t.getWall(d) == null) {
									pos = pos.multiply(Matrix4f.rotate(180, 0,
											1, 0));
									break forloop;
								}
								break;
							case EAST:
								if (t.getWall(d) == null) {
									pos = pos.multiply(Matrix4f.rotate(270, 0,
											1, 0));
									break forloop;
								}
								break;
							...
							case WEST:
								if (t.getWall(d) == null) {
									pos = pos.multiply(Matrix4f.rotate(90, 0,
											1, 0));
									break forloop;
								}
								break;
	...
	private void resetAssets() {
		...
		topDoor = AssetLoader.instance().loadOBJ("res/top_door.obj",
				new Vector3f(0.45f, 0.29f, 0.16f));
		bottomDoor = AssetLoader.instance().loadOBJ("res/bottom_door.obj",
				new Vector3f(0.45f, 0.29f, 0.16f));
		leftDoor = AssetLoader.instance().loadOBJ("res/left_door.obj",
				new Vector3f(0.45f, 0.29f, 0.16f));
		rightDoor = AssetLoader.instance().loadOBJ("res/right_door.obj",
				new Vector3f(0.45f, 0.29f, 0.16f));
		...
		sculpture1 = AssetLoader.instance().loadOBJ("res/sculpture_david.obj",
				new Vector3f(1, 1, 1));
		crates = AssetLoader.instance().loadOBJ("res/crates.obj",
				new Vector3f(0.45f, 0.29f, 0.16f));
		loos = AssetLoader.instance().loadOBJ("res/loo.obj",
				new Vector3f(1, 1, 1));
		sinks = AssetLoader.instance().loadOBJ("res/sink.obj",
				new Vector3f(1, 1, 1));
		desks = AssetLoader.instance().loadOBJ("res/desk.obj",
				new Vector3f(0.95f, 0.95f, 0.95f));

		playerSprite = new AnimatedSpriteController(AssetLoader.instance()
				.loadSpritesheet("res/red_player.png", 32));
		guardSprite = new AnimatedSpriteController(AssetLoader.instance()
				.loadSpritesheet("res/guard.png", 32));
	}
	...
	public void rotateLeft() {
		if (cameraTween == null) {
			cameraTween = new TweenFloat(currentCameraAngle, 0.5f,
					90 * (Math.round((currentCameraAngle + 90) / 90)),
					currentTime);
		}
	}
